 ...
 */
export class BranchFlowSidebarProvider
  implements vscode.WebviewViewProvider, vscode.Disposable
{
  public static readonly viewType = 'branchflow.dashboard';
  ...
  public async refresh(): Promise<void> {
    ...
    const state = await CurrentStateService.resolve();
  ...
  private render(
    state: Awaited<ReturnType<typeof CurrentStateService.resolve>>
  ): string {
    const nonce = getNonce();
    const badges = [
      this.renderBadge(state.policyStatus, this.policyTone(state.policyStatus)),
      this.renderBadge(
        state.workingTreeClean === undefined
      ...
      this.renderBadge(
        state.branchType ? `Type: ${state.branchType}` : 'Type: Unknown',
        'slate'
      )
    ].join('');
...
<html lang="en">
  ...
  private renderActionGrid(availableActions: BranchFlowAction[]): string {
    ...
    const actionMarkup = PRIMARY_ACTIONS.filter(({ action }) =>
      availableActions.includes(action)
    )
      .map(({ action, caption, accent }) => {
        return `
          <button class="action-button ${accent}" data-command="branchflow.${action}">
            <strong>${escapeHtml(BRANCH_FLOW_ACTION_TITLES[action])}</strong>
